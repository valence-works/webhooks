...
- Subscriptions with payload filters must declare explicit `AND`/`OR` matching mode.
- Payload field-path expressions must conform to supported selector syntax.

## Defaults
- Queue-full policy defaults to fail-fast when unset.
...
- Runtime delivery failures are captured in delivery outcome records.
